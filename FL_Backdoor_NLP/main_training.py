...
if __name__ == '__main__':
    ...
    if helper.params['model'] == 'LSTM':
        if helper.params['run_name'] is None:
            wandb.init(entity='imomoe', project=f"backdoor_nlp_{dataset_name}_{model_name}_update", config=helper.params)
        else:
            wandb.init(name=helper.params['run_name'], entity='imomoe', project=f"backdoor_nlp_{dataset_name}_{model_name}_update", config=helper.params)
    else:
        ...
        wandb.init(entity='imomoe', project=f"GPT2_Update_Pred_Last1Word_NumCleanData30_FindRatio", name=wandb_exper_name)
